...
package nodeManager.ContainerExecutor;
...
import nodeManager.launcher.ContainerLaunch;
...
public class LinuxContainerExecutor extends ContainerExecutor {
	 ...
	 */
	enum Commands {
		INITIALIZE_CONTAINER(0), LAUNCH_CONTAINER(1), SIGNAL_CONTAINER(2), DELETE_AS_USER(
				3);
	...
	/**
	 * Result codes returned from the C container-executor. These must match the
	 * values in container-executor.h.
	 */
	enum ResultCode {
		OK(0), INVALID_USER_NAME(2), UNABLE_TO_EXECUTE_CONTAINER_SCRIPT(7), INVALID_CONTAINER_PID(
				9), INVALID_CONTAINER_EXEC_PERMISSIONS(22), INVALID_CONFIG_FILE(
				24), WRITE_CGROUP_FAILED(27);
	...
	public void init() throws IOException {
		...
		List<String> command = new ArrayList<String>(Arrays.asList(
				containerExecutorExe, "--checksetup"));
		String[] commandArray = command.toArray(new String[command.size()]);
		ShellCommandExecutor shExec = new ShellCommandExecutor(commandArray);
		if (LOG.isDebugEnabled()) {
			LOG.debug("checkLinuxExecutorSetup: "
		...
		try {
			...
			throw new IOException(
					"Linux container executor not configured properly"
							+ " (error=" + exitCode + ")", e);
		}
	...
	@Override
	public int launchContainer(Container container, Path nmPrivateContainerDir,
			// Path nmPrivateTokensPath,
			String user, String appId, Path containerWorkDir,
			Path applicationResouceDir, List<String> localDirs
	// List<String> logDirs
	) throws IOException {
		...
		/** 将application resource复制到工作目录 **/
		FileContext lfs = new FileContext();
		// lfs.mkdir(containerWorkDir.toString());
		// Path execFilePath=new
		// Path(localDirs.get(0)+"/nmPrivate/"+appId+"/"+containerIdStr+"/cgroup.jar");
		// Path execFileWorkPath=new Path(containerWorkDir+"/cgroup.jar");
		...
		Path launchDstFile = new Path(containerWorkDir,
				ContainerLaunch.CONTAINER_SCRIPT);
		lfs.copy(nmPrivateCotainerScriptFile.toString(),
				launchDstFile.toString());
		// try {
	...
	public boolean signalContainer(String user, String pid, Signal signal)
		...
		// + "; exit = " + ret_code, e);
		// logOutput(shExec.getOutput());
		// throw new IOException("Problem signalling container " + pid +
		// " with "
	...
	public void mountCgroups(List<String> cgroupKVs, String hierarchy)
			throws IOException {
		List<String> command = new ArrayList<String>(Arrays.asList(
				containerExecutorExe, "--mount-cgroups", hierarchy));
