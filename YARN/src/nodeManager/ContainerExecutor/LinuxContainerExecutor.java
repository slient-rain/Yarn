...
public class LinuxContainerExecutor extends ContainerExecutor {
	...
	@Override
	public int launchContainer(
			Container container,
			Path nmPrivateContainerDir,
			//			Path nmPrivateTokensPath,
			String user, 
			String appId,
			Path containerWorkDir,
			Path applicationResouceDir,
...
//		Path execFilePath=new Path(localDirs.get(0)+"/nmPrivate/"+appId+"/"+containerIdStr+"/cgroup.jar");
...
//		lfs.copy(execFilePath.toString(), execFileWorkPath.toString());
		...
		/**copy launch script to work dir**/
		Path nmPrivateCotainerScriptFile=new Path(nmPrivateContainerDir,ContainerLaunch.CONTAINER_SCRIPT);
		Path launchDstFile =new Path(containerWorkDir, ContainerLaunch.CONTAINER_SCRIPT);
		lfs.copy(nmPrivateCotainerScriptFile.toString(), launchDstFile.toString());
		//		try {
		...
		// DEBUG
		System.out.println("launchContainer: " + Arrays.toString(commandArray));
		shExec.execute();
		if (LOG.isDebugEnabled()) {
			logOutput(shExec.getOutput());
		}
