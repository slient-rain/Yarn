		...
		public ContainerState transition(ContainerImpl container,
			...
			if (!cntrRsrc.isEmpty()) {
				...
				for (Map.Entry<String,LocalResource> rsrc : cntrRsrc.entrySet()) {
					...
					//					LocalResourceVisibility visibility=LocalResourceVisibility.valueOf(LocalResourceVisibility.class, rsrc.getKey());
					//					if (req.get(visibility)==null){
					//						Collection<LocalResourceRequest> collection=new ArrayList<LocalResourceRequest>();
					//						collection.add(new LocalResourceRequest(rsrc.getValue()));
					//						req.put(visibility, collection);
					//					}else {
					//						req.get(visibility).add(new LocalResourceRequest(rsrc.getValue()));
					//					}
				}
				req.put(LocalResourceVisibility.PUBLIC, collection);
				System.out.println("ContainerImpl.RequestResourcesTransition:RequestResourcesTransition.request"+req);

			}


			container.dispatcher.getEventHandler().handle(
					new LocalizerResourceRequestEvent(
							req,
							LocalResourceVisibility.PUBLIC,
							new LocalizerContext(
									container.getUser(),
